...
(function () {
    const coreWeapons = [
        ...
        ,{
            name: "ivory-ebony akimbo",
            descriptionFunction() { return "Auto-combines when both ivory and ebony are owned; dual-wield rapid burst"; },
            ammo: 40,
            ammoPack: 8,
            defaultAmmoPack: 8,
            have: false,
            fire() {
                ...
                for (let i = -2; i <= 2; i++) {
                    const a = base + i * 0.06;
                    b.nail({ x: m.pos.x + (i % 2 ? 8 : -8), y: m.pos.y }, { x: Math.cos(a) * 56, y: Math.sin(a) * 56 }, 13);
                }
                b.explosion(simulation.mouseInGame, 28);
            },
            do() { }
        }
    ];
    ...
    if (Array.isArray(simulation?.ephemera) && !simulation.ephemera.find(e => e.name === 'ivory-ebony-combo')) {
        simulation.ephemera.push({
            name: 'ivory-ebony-combo',
            do() {
                const ivory = b.guns.find(g => g.name === 'ivory');
                const ebony = b.guns.find(g => g.name === 'ebony');
                const akimbo = b.guns.find(g => g.name === 'ivory-ebony akimbo');
                if (!ivory || !ebony || !akimbo) return;
                if (ivory.have && ebony.have && !akimbo.have) {
                    akimbo.have = true;
                    const idx = b.guns.findIndex(g => g.name === 'ivory-ebony akimbo');
                    if (idx !== -1 && !b.inventory.includes(idx)) b.inventory.push(idx);
                    simulation.makeGunHUD?.();
                    simulation.inGameConsole("<span style='color:#f8f9fa'>IVORY + EBONY fused: AKIMBO online</span>", 120);
                }
            }
        });
    }
